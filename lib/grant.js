...
function Grant (config, req, res, next) {
  ...
  this.req = req;
  this.res = res;
  this.domain = req.headers.domain
 ...
 */
function Client (id, secret, domain) {
  this.clientId = id;
...
function credsFromBasic (req) {
  ...
  return new Client(user.name, user.pass, req.headers.domain);
}
...
function credsFromBody (req) {
  return new Client(req.body.client_id, req.body.client_secret, req.headers.domain);
}
...
function checkClient (done) {
  this.model.getClient(this.client.clientId, this.client.clientSecret, this.client.clientDomain,
      function (err, client) {
...
function usePasswordGrant (done) {
  ...
  var uname = this.req.body.username,
    ...
    domain = this.req.headers.domain;
  if (!uname || !pword || !domain) {
    return done(error('invalid_client',
      'Missing parameters. "username", "password", "domain" are required'));
  }

  var self = this;
  return this.model.getUser(uname, pword, domain, function (err, user) {
    if (err) return done(error('server_error', false, err));
    if (!user) {
...
function useRefreshTokenGrant (done) {
  ...
  var self = this;
  this.model.getRefreshToken(token, self.client.clientDomain, function (err, refreshToken) {
    if (err) return done(error('server_error', false, err));
...
function useClientCredentialsGrant (done) {
  ...
  var clientId = this.client.clientId,
    ...
    domain = this.client.clientDomain;
  ...
  var self = this;
  return this.model.getUserFromClient(clientId, clientSecret, domain,
      function (err, user) {
    if (err) return done(error('server_error', false, err));
    if (!user) {
...
function checkGrantTypeAllowed (done) {
  this.model.grantTypeAllowed(this.client.clientId, this.grantType,q
...
function saveAccessToken (done) {
  ...
  this.model.saveAccessToken(accessToken, this.client.clientId, expires, this.user, this.client.clientDomain, function (err) {
    if (err) return done(error('server_error', false, err));
    done();
  });
}
...
function saveRefreshToken (done) {
  ...
  this.model.saveRefreshToken(refreshToken, this.client.clientId, this.user, expires, this.client.clientDomain, function (err) {
    if (err) return done(error('server_error', false, err));
    done();
  });
}
